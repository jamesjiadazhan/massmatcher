...
#' @description
#'
#' `mass_match` first filters the selected metabolite database by monoisotopic
#' mass with a wide mass window, then performs precise m/z matching within the
#' specified ppm threshold.
#' 
...
mass_match = function(unknown_feature, mz_ppm = 10, adduct = c("M+H"), database = c("metorigindb", "pubchem")){
    ...
    mass_for_mz = mass_candidate$Mono_mass
    ...
    mass_for_mz[non_neutral_mask] = mass_candidate$Mono_mass[non_neutral_mask] -
