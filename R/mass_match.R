#' @title Match m/z values to candidate metabolites by monoisotopic mass
#' @description
#'
#' `mass_match` first applies the same broad exact-mass database prefiltering
#' strategy used by `mz_match_clustering()` when `metabolite_database` is not
#' supplied, then performs exact m/z matching within the specified ppm
#' threshold.
#' 
...
#'   provided, `mass_match()` uses it instead of querying the full selected
#'   database.
#' @return A data frame of unknown features matched with molecules from the
#'   selected database based on the specified m/z ppm threshold and adducts.
#' @export mass_match

mass_match = function(unknown_feature, mz_ppm = 10, adduct = c("M+H"), database = c("metorigindb", "pubchem"), metabolite_database = NULL){
    ...
    mass_candidate_theoretical_mz = mass2mz_df_safe(
        mass = mass_candidate$Exact_mass,
